...
export const getLevelBadge = (levelId?: string): { badge: string; badgeClass: string } => {
  ...
  const match = NURSING_LEVEL_OPTIONS.find(
    (o) => o.id === levelId || o.badge.toLowerCase() === levelId.toLowerCase()
  );
  ...
  const legacyMap: Record<string, { badge: string; badgeClass: string }> = {
    'lvl-nd1': { badge: 'ND1', badgeClass: 'bg-blue-50 text-blue-700 border-blue-200' },
    'lvl-nd2': { badge: 'ND2', badgeClass: 'bg-indigo-50 text-indigo-700 border-indigo-200' },
  ...
  return legacyMap[levelId] || { badge: levelId, badgeClass: 'bg-slate-100 text-slate-700 border-slate-200' };
};
