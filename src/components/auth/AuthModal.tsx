...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const { login, register, forgotPassword, confirmPasswordResetAction, resetPassword } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'login' || mode === 'admin_login') {
        ...
        if (!resetCode.trim()) {
          throw new Error('Please enter the reset code or action link.');
        }
        ...
        try {
          await confirmPasswordResetAction(resetCode.trim(), newPassword);
        } catch {
          // Fallback to legacy endpoint if code was generated by server
          await resetPassword({
            email: cleanEmail,
  ...
  const handleResendCode = async () => {
    ...
    try {
      ...
      setResendCooldown(60);
      setSuccessMsg('A password reset link has been resent. Please check your inbox and spam folder.');
    } catch (err: any) {
      setError(getFirebaseAuthErrorMessage(err));
    } finally {
      setIsSubmitting(false);
          ...
          */}
          {mode === 'reset_password' && (
            <div className="space-y-3.5">
              ...
              {/* 2. Action Code or Verification Code */}
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-300">
                    Reset Code / Action Code <span className="text-teal-400">*</span>
                  </label>
                  <button
                    type="button"
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Enter code from email or URL"
                    className="w-full pl-10 pr-3 py-2 bg-slate-900 border border-slate-800 rounded-xl text-xs text-white font-mono placeholder:tracking-normal placeholder:font-sans placeholder:text-slate-500 focus:outline-none focus:border-teal-500 transition-colors"
                  />
                </div>
              </div>
